...
public final class HealthBar {
    ...
    private static final int LAYER_GHOST = 1;
    private static final int LAYER_FILL = 2;
    ...
    private static final float CRITICAL_THRESHOLD = 0.4f;
    ...
    public void render(GuiGraphics graphics) {
        ...
        boolean criticalHealth = healthPercent < CRITICAL_THRESHOLD;
        ...
        if (blink > 0f) {
            RenderSystem.setShaderColor(1f, 1f, 1f, blink * healthAlpha);
            drawHealth(graphics, LAYER_OUTLINE, x, y, vState, 1f);
        }

        RenderSystem.setShaderColor(1f, 1f, 1f, healthAlpha);
        drawHeart(graphics, x, y, state, low, blink);
    ...
    private void drawHeart(GuiGraphics graphics, int x, int y, int state, boolean low, float blink) {
        int hx = x - HEART / 2;
